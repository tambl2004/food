...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Chỉnh sửa món ăn</h2>
            <p class="text-gray-600 mt-1">Cập nhật thông tin món ăn và nguyên liệu</p>
        </div>
        <div>
    ...
    <form method="POST" action="{{ route('admin.dishes.update', $dish) }}" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" 
                              rows="4" 
                              required
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">{{ old('description', $dish->description) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <input type="url" 
                           name="video_url" 
                           value="{{ old('video_url', $dish->video_url) }}" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div>
                    ...
                    <select name="difficulty" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                        ...
                        <option value="easy" {{ old('difficulty', $dish->difficulty) == 'easy' ? 'selected' : '' }}>Dễ</option>
                        <option value="medium" {{ old('difficulty', $dish->difficulty) == 'medium' ? 'selected' : '' }}>Trung bình</option>
                        <option value="hard" {{ old('difficulty', $dish->difficulty) == 'hard' ? 'selected' : '' }}>Khó</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" 
                           name="prep_time" 
                           value="{{ old('prep_time', $dish->prep_time) }}" 
                           min="0"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                </div>
                ...
                <div>
                    ...
                    <input type="number" 
                           name="cook_time" 
                           value="{{ old('cook_time', $dish->cook_time) }}" 
                           required
                           min="1"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                </div>
                ...
                <div>
                    ...
                    <input type="number" 
                           name="servings" 
                           value="{{ old('servings', $dish->servings) }}" 
                           min="1"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                </div>
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            ...
            <div id="ingredients-container" class="space-y-4">
                @php
                    $existingIngredients = old('ingredients') ?? $dish->ingredients->map(function($ing) {
                        return [
                            'ingredient_id' => $ing->id,
                            'quantity' => $ing->pivot->quantity,
                            'unit' => $ing->pivot->unit,
                            'is_required' => $ing->pivot->is_required,
                        ];
                    })->toArray();
                @endphp
...
@section('scripts')
<script>
let ingredientIndex = {{ old('ingredients') ? count(old('ingredients')) : (isset($existingIngredients) ? count($existingIngredients) : 1) }};
