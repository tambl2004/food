...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            ...
            <p class="text-gray-600 mt-1">Cập nhật thông tin danh mục món ăn</p>
        </div>
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" 
                              ...
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">{{ old('description', $category->description) }}</textarea>
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="meal_type" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                        ...
                        <option value="breakfast" {{ old('meal_type', $category->meal_type) == 'breakfast' ? 'selected' : '' }}>Sáng</option>
                        <option value="lunch" {{ old('meal_type', $category->meal_type) == 'lunch' ? 'selected' : '' }}>Trưa</option>
                        <option value="dinner" {{ old('meal_type', $category->meal_type) == 'dinner' ? 'selected' : '' }}>Tối</option>
                        <option value="snack" {{ old('meal_type', $category->meal_type) == 'snack' ? 'selected' : '' }}>Đồ ăn vặt</option>
                        <option value="dessert" {{ old('meal_type', $category->meal_type) == 'dessert' ? 'selected' : '' }}>Tráng miệng</option>
                    </select>
                ...
                <div>
                    ...
                    <select name="diet_type" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                        ...
                        <option value="vegetarian" {{ old('diet_type', $category->diet_type) == 'vegetarian' ? 'selected' : '' }}>Chay</option>
                        <option value="vegan" {{ old('diet_type', $category->diet_type) == 'vegan' ? 'selected' : '' }}>Thuần chay</option>
                        <option value="keto" {{ old('diet_type', $category->diet_type) == 'keto' ? 'selected' : '' }}>Keto</option>
                        <option value="low-carb" {{ old('diet_type', $category->diet_type) == 'low-carb' ? 'selected' : '' }}>Ít carb</option>
                        <option value="gluten-free" {{ old('diet_type', $category->diet_type) == 'gluten-free' ? 'selected' : '' }}>Không gluten</option>
                        <option value="halal" {{ old('diet_type', $category->diet_type) == 'halal' ? 'selected' : '' }}>Halal</option>
                        <option value="regular" {{ old('diet_type', $category->diet_type) == 'regular' ? 'selected' : '' }}>Thông thường</option>
                    </select>
